...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            #create every action as a PgNode_a object
            node_a = PgNode_a(action)

            #determine the action to the action_nodes
    ...
    def add_literal_level(self, level):
        ...
        #for every action_nodes in the previous level

        for node_a in self.a_levels[level - 1]:

            #add state node from the node_a.effnodes to the set of state_nodes
            for node_s in node_a.effnodes:
                state_nodes.add(node_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #if every action effect_add is in the other node's effect_rem then return True

        #action  effect_add in node_a1
        for pos_1 in node_a1.action.effect_add:
            if pos_1 in node_a2.action.effect_rem:
                return True
        #action effect_add in node_a2
        for pos_2 in node_a2.action.effect_add:
            if pos_2 in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #make sure that every node's effect_add is not same as the other's precond_neg
        for pos_1 in node_a1.action.effect_add:
            if pos_1 in node_a2.action.precond_neg:
                return True

        for pos_2 in node_a2.action.effect_add:
            if pos_2 in node_a1.action.precond_neg:
                return True

        #make sure that every node's effect_rem is not same as the other's precond_pos
        for neg_1 in node_a1.action.effect_rem:
            if neg_1 in node_a2.action.precond_pos:
                return True

        for neg_2 in node_a2.action.effect_rem:
            if neg_2 in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #make sure that every node's precond is not same as the other's precond_neg
